...
function logHtmlRenderer(message, level = 'debug') {
    if (typeof window.logMessage === 'function') {
        window.logMessage(`[HtmlRenderer] ${message}`, level, 'HTML_RENDERER');
    } else {
        console.log(`[HtmlRenderer] ${message}`);
    }
}
...
export class HtmlRenderer {
    ...
    async render(htmlContent, filePath) {
        ...
        logHtmlRenderer(`Successfully generated iframe for HTML file: ${filePath}`);
        return result;
    }
    ...
    async postProcess(previewElement, renderResult, filePath) {
        logHtmlRenderer('Post-processing for iframe-based HTML render.');
    ...
    ensureIframeIsolation(iframe) {
        ...
        logHtmlRenderer('Applied CSS isolation to iframe');
    }
    ...
    setupIframeEventListeners(iframe, container, toolbar) {
        iframe.addEventListener('load', () => {
            logHtmlRenderer('HTML iframe loaded successfully');
        ...
        iframe.addEventListener('error', (e) => {
            logHtmlRenderer(`HTML iframe error: ${e.message}`, 'error');
    ...
    analyzeIframeContent(iframe) {
        try {
            ...
            if (iframeDoc) {
                ...
                logHtmlRenderer(`Iframe content analyzed - Title: ${title}, Body preview: ${bodyText}...`);
            }
        } catch (e) {
            logHtmlRenderer('Cannot analyze iframe content (cross-origin or security restrictions)');
        }
    }
    ...
    openCssDebugger(iframe, filePath) {
        logHtmlRenderer('Opening CSS debugger for iframe');
    ...
    createCssDebugPanel(iframe, filePath) {
        logHtmlRenderer('Opening CSS debug - redirecting to CSS Files Panel');
        ...
        import('/client/eventBus.js').then(({ eventBus }) => {
            ...
            logHtmlRenderer(`Failed to open CSS Files Panel: ${error.message}`, 'error');
        ...
        import('/client/eventBus.js').then(({ eventBus }) => {
            eventBus.on('css:changed', (eventData) => {
                ...
                if (eventType === 'cssToggled') {
                    logHtmlRenderer(`CSS file toggled via EventBus: ${data.href} (enabled: ${data.enabled})`);
     ...
     */
    destroy() {
        logHtmlRenderer('Cleaning up HTML renderer');
