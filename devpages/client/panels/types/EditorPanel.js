...
export class EditorPanel extends BasePanel {
        ...
        try {
            ...
            if (storedAuth) {
                ...
                if (parsedAuth.isLoggedIn && parsedAuth.expiresAt > Date.now()) {
                    this.log('Found valid auth state in localStorage');
                    ...
                    if (!document.body.hasAttribute('data-app-state') || document.body.getAttribute('data-app-state') !== 'ready') {
                        setTimeout(() => this.triggerFileManagerInit(parsedAuth.username), 100);
                    }
                }
            }
        } catch (error) {
    ...
    setValue(content) {
        if (this.textarea) {
            ...
            // Only trigger change event if content actually changed
