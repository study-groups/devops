...
export class JavaScriptPanel extends BasePanel {
    ...
    async initialize() {
        ...
        try {
            // Import and initialize the highlight plugin
            const { HighlightPlugin } = await import('/client/preview/plugins/highlight.js');
            this.highlightPlugin = new HighlightPlugin({ theme: 'light' });
            ...
            this.log(`JavaScriptPanel initialization failed: ${error.message}`, 'error');
            return false;
        }
    }
    ...
    async performJavaScriptUpdate(content, filePath) {
        ...
        try {
            ...
            if (this.highlightPlugin && this.highlightPlugin.isReady()) {
                const language = this.getLanguageFromPath(filePath);
                const highlightedCode = this.highlightPlugin.highlight(content, language);
                this.codeElement.innerHTML = highlightedCode;
                this.codeElement.classList.add('hljs');
                this.log(`Applied ${language} syntax highlighting`);
            } else {
            ...
            if (this.jsState.renderQueue.length > 0) {
                const nextUpdate = this.jsState.renderQueue.shift();
                setTimeout(() => this.performJavaScriptUpdate(nextUpdate.content, nextUpdate.filePath), 100);
            }
        }
    }
