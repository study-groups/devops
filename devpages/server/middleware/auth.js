...
export const authMiddleware = (req, res, next) => {
    console.log(`[AUTH MIDDLEWARE] ${req.method} ${req.path} - Checking authentication status...`);
    ...
    console.log(`[AUTH MIDDLEWARE] req.session:`, req.session ? { id: req.session.id, user: req.session.user } : 'No session');
 ...
 * @returns {Promise<object>} Token data {token, expiresAt, username}
 */
export async function generatePDataToken(req, expiryHours = 24, description = 'API Access Token') {
    ...
    try {
        // Create PData token for already authenticated user (bypasses password validation)
        const token = await req.pdata.createTokenForAuthenticatedUser(username);
        
        if (!token) {
            throw new Error('Failed to create PData token');
        }
