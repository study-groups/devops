        ...
        try {
            ...
            if (storedState) {
                ...
                results.success = fieldsToCompare.every(field => {
                    const modifiedValue = modifiedState[field];
                    const storedValue = payload[field];
                    if (typeof modifiedValue === 'object' && modifiedValue !== null) {
                        return JSON.stringify(modifiedValue) === JSON.stringify(storedValue);
                    }
                    return storedValue === modifiedValue;
                });
                ...
                fieldsToCompare.forEach(field => {
                    ...
                    let isEqual;
                    if (typeof modifiedValue === 'object' && modifiedValue !== null) {
                        isEqual = JSON.stringify(modifiedValue) === JSON.stringify(storedValue);
                    } else {
                        isEqual = storedValue === modifiedValue;
                    }
                    console.log(`${field}:`, 
                        `Modified: ${JSON.stringify(modifiedValue)}`, 
                        `Stored: ${JSON.stringify(storedValue)}`, 
                        isEqual ? '✅' : '❌'
                    );
                });
                console.groupEnd();
