...
class DebugPanelManager {
    ...
    async init() {
        ...
        // Panels are now loaded on show() to ensure they are always up-to-date
    ...
    loadPanels() {
        // First, clear out any existing panel content and instances to prevent duplication
        ...
        console.log('[DebugPanelManager] Loading debug panels:', debugPanels.map(p => ({ id: p.id, title: p.title, hasComponent: !!p.component })));
        ...
        debugPanels.forEach(panelConfig => {
            ...
            console.log(`[DebugPanelManager] Processing panel ${sectionId}:`, {
                title: panelConfig.title,
                hasComponent: !!panelConfig.component,
                componentType: panelConfig.component ? panelConfig.component.name : 'undefined',
                isVisible,
                isCollapsed
            });
            ...
            if (!isVisible) {
                ...
                console.log(`[DebugPanelManager] Skipping hidden panel: ${sectionId}`);
                return;
            }
            ...
            if (PanelComponent) {
                try {
                    console.log(`[DebugPanelManager] Creating instance for panel ${sectionId} with component:`, PanelComponent.name);
                    this.sectionInstances[sectionId] = new PanelComponent(content);
                    console.log(`[DebugPanelManager] Successfully created instance for panel ${sectionId}`);
                } catch (error) {
                    console.error(`[DebugPanelManager] Error creating instance for panel ${sectionId}:`, error);
                }
            } else {
                console.error(`No component found for debug panel: ${sectionId}`, {
                    panelConfig,
                    availableProps: Object.keys(panelConfig)
                });
            }
        });
    }
    ...
    show() {
        // Ensure panels are loaded every time the panel is shown.
        this.loadPanels();
