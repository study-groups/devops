/**
 * @file client/settings/panels/api-tokens/ApiTokenPanel.js
...
export class ApiTokenPanel extends BasePanel {
    constructor(options) {
        super(options);
    ...
    render() {
        if (!this.element) {
            ...
            this.element.className = 'api-token-panel';
        }
    ...
    onMount(container) {
        ...
        this.log('PANEL_INIT', 'INITIALIZED', 'ApiTokenPanel initialized');
    }
    ...
    attachEventListeners() {
        // Token generation
        this.element.querySelector('#generate-token-btn')?.addEventListener('click', () => {
            this.generateToken();
        });

        // Copy token
        this.element.querySelector('#copy-token-btn')?.addEventListener('click', () => {
            this.copyToken();
        });

        // Refresh tokens
        this.element.querySelector('#refresh-tokens-btn')?.addEventListener('click', () => {
            this.loadTokens();
        });

        // Test token
        this.element.querySelector('#test-token-btn')?.addEventListener('click', () => {
            this.testToken();
        });
    }

    async generateToken() {
        const expiryHours = parseInt(this.element.querySelector('#token-expiry').value);
        const description = this.element.querySelector('#token-description').value.trim() || 'API Access Token';
    ...
    displayNewToken(tokenData) {
        const displaySection = this.element.querySelector('#token-display-section');
        const tokenInput = this.element.querySelector('#generated-token');
        const curlExample = this.element.querySelector('#curl-example');
        const jsExample = this.element.querySelector('#js-example');
    ...
    async loadTokens() {
        const tokensList = this.element.querySelector('#tokens-list');
    ...
    async testToken() {
        const endpoint = this.element.querySelector('#test-endpoint').value;
