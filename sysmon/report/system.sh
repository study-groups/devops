#!/bin/bash
# generate_system_report.sh - Connects to remote systems, collects system info, and generates a Mermaid diagram.
...
collect_remote_info() {
    ...
    scp "${SSH_USER}@${server}:system_status_$(hostname).txt" "${LOCAL_OUTPUT_DIR}/system_status_${server}.txt"
...
# Add connections
echo "  do4 --- do4_n2" >> "$MERMAID_OUTPUT"
