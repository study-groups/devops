...
sm_routes_nginx_sites_summary(){
  ...
  for site in "${enabled_sites[@]}"; do
    ...
    # Collect subdomains
    local domain=$(ssh root@$1 "awk '/server_name/ {for (i=2; i<=NF; i++) print \$i; while (getline > 0 && \$0 ~ /^[[:space:]]+/) { print \$1; }}' /etc/nginx/sites-available/$site" | tr '\n' ' ')
    domain=$(echo $domain | xargs) # Trim leading and trailing whitespace
    domain=$(echo $domain | sed 's/ *\*[^ ]*//g') # Remove wildcard domains
    IFS=' ' read -r -a site_subdomains <<< "$domain"
    subdomains+=("${site_subdomains[@]}")
    subdomain_count=$((subdomain_count + ${#site_subdomains[@]}))
  ...
  echo "----------------------------------------"
  echo "Total subdomains served: $subdomain_count"
...
sm_routes_summary(){
  local ip=$1
  local hostname=$(ssh root@$ip "hostname") # Fetch hostname
  echo "Investigating endpoints on server: $ip ($hostname)" # Show hostname
  
  # List enabled sites
  echo "Nginx sites-enabled on server: $ip ($hostname)"
  sm_routes_nginx_enabled $ip
  
  # List available sites and provide summaries
  echo "Nginx sites summary on server: $ip ($hostname)"
...
sm_routes_nginx_subdomain_summary(){
  local enabled_sites=($(sm_routes_nginx_enabled $1))
  local subdomain_count=0
  local subdomains=()
