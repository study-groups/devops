# Collects original data from stdin
# Places into array in JSON
# Data[] => JSON-String[]
data-to-json-array-strings() {
  local original_data=()
  ...
  do
    [ "$i" -lt "$((len-1))" ] && json+="\""${original_data[$i]}"\"" && json+=","
    [ "$i" -eq "$((len-1))" ] && json+="\""${original_data[$i]}"\"" && json+="]}"
  done
...
##Later: Line[] => Maybe Sentiment[] NaN[]
# Current: Line[] => Sentiment[]
lines-to-sentiments(){
 ...
 # json to be sent to api
  local json="$(data-to-json-array-strings)";
 # (parse range | map-data-to-json)
...
sentiments-to-average() {
  ...
  # ignores NaNs for specific average
