...
(function(ns) {
    ...
    if (ns.log && ns.log.logToServer) {
        ns.log.info('frontend.devwatch-iframe', 'Advanced logger already present. Skipping simple logger initialization.');
        return;
    }

    // If no advanced logger, define a simple fallback.
    const log = (level, message, data) => {
        const timestamp = new Date().toLocaleTimeString();
        console[level](`[${timestamp}] [PJA ${level.toUpperCase()}]`, message, data || '');
    };
...
class DevWatchSdk {
    constructor(options = {}) {
        ...
        if (this.isIframe && this.config.enableSecurity) {
        ...
        if (this.isIframe) {
            ...
            APP.log.warn('frontend.devwatch-iframe', 'Not running in an iframe context. SDK will initialize with limited functionality.');
    ...
    init() {
        this.sendMessage('devwatch-iframe-ready', { url: window.location.href });
        ...
        APP.log.info('frontend.devwatch-iframe', 'Iframe SDK Initialized.');
    }
    ...
    initStandalone() {
        ...
        APP.log.info('frontend.devwatch-iframe', 'PJA SDK initialized in standalone mode.');
    }
    ...
    handleHostMessage(event) {
        ...
        if (type === 'devwatch-set-theme') {
            ...
            this.sendMessage('devwatch-iframe-test-response', {
    ...
    sendTitle() {
        ...
        this.sendMessage('devwatch-title-update', { title: title });
    }
    ...
    setTheme(theme) {
        if (theme) {
            ...
            APP.log.info('frontend.devwatch-iframe', `Theme set to: ${theme}`);
        }
    }
    ...
    sendAssetLists() {
        try {
            ...
            if (window.DevWatchAssets) {
                window.DevWatchAssets.css = cssFiles;
                window.DevWatchAssets.js = jsFiles;
            }
    ...
    initializeActivityLog() {
        this.logContainer = document.getElementById('activity-log') || document.querySelector('.devwatch-status-display');
    ...
    addLogEntry(message, type = 'info', data = {}) {
        ...
        APP.log.info('frontend.devwatch-iframe', message, data);
    ...
    validateDomain() {
        try {
            ...
            if (!referrer) {
                // Allow if no referrer (direct access in development)
                return window.location.hostname === 'localhost';
            }

            const isValidDomain = this.config.allowedDomains.some(domain => 
                referrer.includes(domain)
            );

            if (!isValidDomain) {
                this.showSecurityWarning('Invalid parent domain');
                return false;
            }

            return true;
        } catch (error) {
            APP.log.error('frontend.devwatch-iframe', 'Security validation failed', error);
            this.showSecurityWarning('Security validation error');
            return false;
        }
    }

    showSecurityWarning(message = 'Unauthorized access') {
        document.body.innerHTML = `
            ...
            ">
                <div>
                    <h2 style="margin-bottom: 16px;">🎮 Pixel Jam Arcade</h2>
                    <p style="margin-bottom: 20px;">${message}</p>
                    <p style="margin-bottom: 20px;">This content must be accessed through our arcade.</p>
                    <a href="https://pixeljamarcade.com" 
                       style="color: #4CAF50; text-decoration: none; font-weight: bold;">
                        Visit Pixel Jam Arcade
                    </a>
                </div>
            </div>
        `;
        ...
        scripts.forEach(script => {
            if (script.src && !this.config.allowedDomains.some(domain => script.src.includes(domain))) {
                script.remove();
            }
        });
    }

    // Authentication check (replaces PJA_GAME_API)
    checkAuth() {
        return new Promise((resolve) => {
            ...
            const authHandler = (event) => {
                if (event.data && event.data.type === 'PJA_AUTH_RESPONSE') {
        ...
        if (detailsContent) {
            ...
            copyBtn.addEventListener('click', async (e) => {
                ...
                try {
                    ...
                    APP.log.warn('frontend.devwatch-iframe', 'Copy failed', err);
                }
            });
        }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const shouldInitialize = window.location.pathname.includes('.iframe.html') || 
                             document.querySelector('meta[name="devwatch-iframe-enabled"]') ||
                             document.body.classList.contains('devwatch-iframe-body') ||
                             window.location.pathname.includes('/games/');
    
    if (shouldInitialize) {
        // Initialize even in standalone if an activity log container exists
        const sdk = new DevWatchSdk();
        
        window.APP.sdk = sdk;
        
        // Create legacy PJA_IFRAME_HELPER for compatibility with games
        window.DevWatch_IFRAME_HELPER = {
            init: (options = {}) => {
                if (options.onReady) {
                    options.onReady();
                }
            ...
            sendTitle: (title) => {
                if (title) {
                    sdk.sendMessage('devwatch-title-update', { title });
                } else {
                    sdk.sendTitle();
                }
            }
        ...
        // Create legacy PJA_GAME_API for compatibility (replaces devwatch-iframe-enhanced.js)
        window.DevWatch_GAME_API = {
            checkAuth: () => sdk.checkAuth()
        };
