...
(function() {
    ...
    var trashBody = document.getElementById('trash-body');
    var trashStatsBar = document.getElementById('trash-stats-bar');
    var trashRefreshBtn = document.getElementById('trash-refresh-btn');
    var trashEmptyBtn = document.getElementById('trash-empty-btn');

    function loadTrash() {
        fetch(API + '/trash?limit=100').then(function(r) { return r.json(); }).then(function(data) {
            renderTrash(data.voxes || [], data.total || 0);
        }).catch(function() {
            renderTrash([], 0);
        });
    }

    function renderTrash(voxes, total) {
        trashBody.innerHTML = '';
        trashStatsBar.textContent = 'Trash: ' + total + ' item' + (total !== 1 ? 's' : '');
        ...
        trashBody.querySelectorAll('.trash-restore-btn').forEach(function(btn) {
            btn.addEventListener('click', function() {
                var vid = btn.getAttribute('data-id');
                btn.textContent = '...';
                btn.disabled = true;
                fetch(API + '/trash/' + vid + '/restore', { method: 'POST' })
                    .then(function(r) { return r.json(); })
                    .then(function(result) {
                        if (result.ok) {
                            loadTrash();
                            loadDb(); // refresh main list
                        } else {
        ...
        trashBody.querySelectorAll('.trash-delete-btn').forEach(function(btn) {
            btn.addEventListener('click', function() {
                var vid = btn.getAttribute('data-id');
                if (!confirm('Permanently delete vox ' + vid + '?')) return;
                btn.textContent = '...';
                btn.disabled = true;
                fetch(API + '/trash/' + vid, { method: 'DELETE' })
                    .then(function(r) { return r.json(); })
                    .then(function(result) {
                        if (result.ok) loadTrash();
                    })
                    .catch(function() {
    ...
    window.VoxTrash = { refresh: loadTrash };
