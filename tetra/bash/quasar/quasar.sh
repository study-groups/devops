...
# ============================================================================
# Local Sound Daemon (FIFO-based, no browser needed)
# ============================================================================

QUASAR_FIFO="${QUASAR_FIFO:-/tmp/pulsar_sound.fifo}"

quasar_local() {
    local engine="${1:-atari2600}"

    # Check if already running
    if pgrep -f "node.*quasar_local" &>/dev/null; then
        echo "Local daemon already running. Use 'quasar local-kill' to stop it."
        return 1
    fi

    echo "Starting local sound daemon (engine: $engine)..."
    node "$QUASAR_SRC/quasar_local.js" --engine "$engine"
}
...
quasar_local_bg() {
    ...
    echo "Starting local daemon in background..."
    nohup node "$QUASAR_SRC/quasar_local.js" --engine "$engine" \
        > "$QUASAR_DIR/local.log" 2>&1 &
    echo "PID: $!"
    echo "Log: $QUASAR_DIR/local.log"
}

quasar_local_kill() {
    local pids
    pids=$(pgrep -f "node.*quasar_local" 2>/dev/null)

    if [[ -z "$pids" ]]; then
        echo "No local daemon running"
        return 0
    fi

    echo "Killing local daemon(s): $pids"
    pkill -f "node.*quasar_local"

    # Wait for processes to die (up to 2 seconds)
    local i=0
    while pgrep -f "node.*quasar_local" &>/dev/null && (( i++ < 20 )); do
        sleep 0.1
    done
    ...
    if pgrep -f "node.*quasar_local" &>/dev/null; then
        echo "Force killing..."
        pkill -9 -f "node.*quasar_local"
        sleep 0.2
    fi

    # Clean up FIFO
    [[ -p "$QUASAR_FIFO" ]] && rm -f "$QUASAR_FIFO"
...
quasar_local_status() {
    ...
    if [[ -n "$pids" ]]; then
        echo "Local daemon running:"
        ps -p "$pids" -o pid,etime,command 2>/dev/null | tail -n +2
        echo ""
        echo "FIFO: $QUASAR_FIFO"
        [[ -p "$QUASAR_FIFO" ]] && echo "  (exists)" || echo "  (missing)"
    else
...
quasar_sound() {
    local sound="${1:-collision}"

    if [[ ! -p "$QUASAR_FIFO" ]]; then
        echo "FIFO not found. Start daemon with: quasar local"
        return 1
    fi

    echo "$sound" > "$QUASAR_FIFO"
}
...
quasar() {
    ...
    case "$cmd" in
        ...
        local)       quasar_local "$@" ;;
        local-bg)    quasar_local_bg "$@" ;;
        local-kill)  quasar_local_kill ;;
        local-status) quasar_local_status ;;
        sound)       quasar_sound "$@" ;;
        help|--help|-h)
            cat <<'EOF'
...
Local Daemon (FIFO-based, no browser):
  local [engine]  Start local sound daemon (foreground)
  local-bg [eng]  Start local daemon in background
  local-kill      Kill local daemon
  local-status    Show local daemon status
  sound <name>    Send sound to local daemon
...
Examples:
  ...
  quasar local sid       # Start local daemon with SID engine
  quasar local-kill      # Kill local daemon
  quasar sound collision # Trigger collision sound
  quasar sound pew       # Trigger pew sound

TIA Engines: atari2600 (default), sid, fm
EOF
            ;;
        *)
