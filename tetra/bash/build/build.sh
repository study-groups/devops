#!/usr/bin/env bash
# build/build.sh - Tetra Build System
#
# Simple file transforms: concat, copy, template
# Config: proj/tsm/tetra-build.toml
#
# Usage:
#   tetra build              Build current directory
#   tetra build -n           Dry run
#   tetra build --remote env Build on remote, artifacts stay there

BUILD_VERSION="1.0.0"

# Module paths
BUILD_SRC="${TETRA_SRC}/bash/build"
...
# Find tetra-build.toml
_build_find_config() {
    local search_dir="${1:-.}"

    # Check tsm/ first (recommended location)
    if [[ -f "$search_dir/tsm/tetra-build.toml" ]]; then
        echo "$search_dir/tsm/tetra-build.toml"
        return 0
    fi

    # Fallback to root
    if [[ -f "$search_dir/tetra-build.toml" ]]; then
        echo "$search_dir/tetra-build.toml"
        return 0
    fi
...
tetra_build() {
    ...
    if [[ -z "$config" ]]; then
        echo "Error: No tetra-build.toml found" >&2
        echo "Searched: $project_dir/tsm/tetra-build.toml, $project_dir/tetra-build.toml" >&2
        return 1
    fi
...
_build_help() {
    echo "tetra build - Simple file transforms"
    echo ""
    echo "Usage:"
    echo "  tetra build              Build current directory"
    echo "  tetra build <dir>        Build specified directory"
    echo "  tetra build -n           Dry run (show what would happen)"
    echo "  tetra build --remote env Build on remote server"
    echo ""
    echo "Config: tsm/tetra-build.toml"
    echo ""
    echo "Example tetra-build.toml:"
    echo "  [build]"
    echo "  name = \"myapp\""
    echo ""
    echo "  [bundles.css]"
