#!/usr/bin/env bash
# cdp.sh - Chrome DevTools Protocol wrapper for RAG agent workflows
#
...
cdp_get_chrome_binary() {
    # Try various Chrome/Chromium paths
    local chrome_paths=(
...
cdp_launch_chrome() {
    ...
    local user_data_dir="${2:-$(mktemp -d)}"
    ...
    # Launch Chrome in headless mode with CDP enabled
    "$chrome_bin" \
...
cdp_execute_action() {
    ...
    # Ensure connected (except for init/connect)
...
cdp_cleanup() {
    ...
    # Clean cache (optional - keep db/ for history)
...
# Export functions
export -f cdp_load_active_profile
export -f cdp_get_chrome_binary
export -f cdp_launch_chrome
export -f cdp_get_ws_url
export -f cdp_connect
export -f cdp_send_command
export -f cdp_navigate
export -f cdp_screenshot
export -f cdp_execute
export -f cdp_execute_js
export -f cdp_get_html
export -f cdp_extract
export -f cdp_click
export -f cdp_type
export -f cdp_disconnect
export -f cdp_kill_chrome
export -f cdp_init
export -f cdp_execute_action
export -f cdp_cleanup
