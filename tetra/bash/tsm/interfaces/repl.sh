...
Built-in Commands:
  ...
  /sessions <cmd>   Session analytics commands (help for list)
...
EOF
    ...
    case "$cmd" in
        summary)
            ...
            echo "══════════════════════════════"
            output=$(source "$TETRA_SRC/bash/tsm/tsm_session_aggregator.sh" && tsm_extract_sessions "$service" 600 summary 2>&1)
            echo "$output"
            ;;
        list)
            echo "📋 Active Sessions for $service"
            echo "═══════════════════════════════"
            output=$(source "$TETRA_SRC/bash/tsm/tsm_session_aggregator.sh" && tsm_extract_sessions "$service" 1800 detailed 2>&1)
            echo "$output"
            ;;
        users)
            echo "👥 User Traffic Analysis for $service"
            echo "════════════════════════════════════"
            output=$(source "$TETRA_SRC/bash/tsm/tsm_session_aggregator.sh" && tsm_disambiguate_users "$service" 1800 2>&1)
            echo "$output"
            ;;
        patterns)
            local user_filter=""
            ...
            if [[ -n "$user_filter" ]]; then
                ...
                echo "═══════════════════════════════════════════════════════"
                output=$(source "$TETRA_SRC/bash/tsm/tsm_session_aggregator.sh" && tsm_analyze_user_patterns "$service" "$user_filter" 2>&1)
            else
                echo "🎯 Behavioral Patterns for $service"
                echo "══════════════════════════════════"
                output=$(source "$TETRA_SRC/bash/tsm/tsm_session_aggregator.sh" && tsm_analyze_user_patterns "$service" 2>&1)
            fi
            echo "$output"
            ;;
        clicks)
            ...
            echo "═══════════════════════════════"
            output=$(source "$TETRA_SRC/bash/tsm/tsm_analytics.sh" && tsm_analyze_click_timing "$service" 300 2>&1)
            echo "$output"
            ;;
        journey)
            local session_id=""
            ...
            if [[ -n "$session_id" ]]; then
                ...
                echo "═══════════════════════════════════════════════════════"
                output=$(source "$TETRA_SRC/bash/tsm/tsm_analytics.sh" && tsm_analyze_user_journey "$service" "$session_id" 1800 2>&1)
            else
                echo "🗺️  Recent User Journeys for $service"
                echo "════════════════════════════════════"
                output=$(source "$TETRA_SRC/bash/tsm/tsm_analytics.sh" && tsm_analyze_user_journey "$service" "" 600 2>&1)
            fi
            echo "$output"
            ;;
        *)
