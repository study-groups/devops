...
# Promote a built .tsm file to services-available
# Usage: tsm promote [path/to/service.tsm]
#
# If no path given, looks for built .tsm files in ./tsm/
# Only promotes files generated by 'tsm build' (contains "Generated from env.toml")
tsm_promote() {
    local tsm_path="$1"

    # If no path given, look in ./tsm/ for built .tsm files
    if [[ -z "$tsm_path" ]]; then
        if [[ -d "./tsm" ]]; then
            local -a built_files=()
            for f in ./tsm/*.tsm; do
                ...
                if grep -q "Generated from env.toml" "$f" 2>/dev/null; then
                    built_files+=("$f")
                fi
            done

            if [[ ${#built_files[@]} -eq 0 ]]; then
                tsm_error "No built .tsm files found in ./tsm/"
                tsm_error "Run 'tsm build <env>' first"
                return 1
            elif [[ ${#built_files[@]} -eq 1 ]]; then
                tsm_path="${built_files[0]}"
            else
                echo "Multiple .tsm files found. Choose one:"
                local i=1
                for f in "${built_files[@]}"; do
                    echo "  $i) $(basename "$f")"
                    ((i++))
                done
                tsm_error "Usage: tsm promote <path/to/service.tsm>"
                return 1
            fi
        else
            tsm_error "No ./tsm/ directory and no path specified"
            return 1
        fi
    fi
    ...
    # Verify it's a built file
    if ! grep -q "Generated from env.toml" "$tsm_path" 2>/dev/null; then
        tsm_error "Not a built .tsm file (missing 'Generated from env.toml' header)"
        tsm_error "Only promote files created by 'tsm build'"
        return 1
    fi

    local name
    name=$(basename "$tsm_path" .tsm)
    local link_path="$TSM_SERVICES_AVAILABLE/${name}.tsm"
