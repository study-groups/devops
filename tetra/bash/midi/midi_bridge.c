...
static void poll_gamepad(void) {
    ...
    /* Also broadcast as OSC for network listeners */
    /* Channel 16 = gamepad virtual CCs */
    for (int i = 0; i < 6; i++) {
        /* Convert int16 axis to 0-127 CC value */
        int cc_value;
        if (i < 4) {
            /* Sticks: -32768..32767 -> 0..127 */
            cc_value = (state.axes[i] + 32768) * 127 / 65535;
        } else {
            /* Triggers: 0..32767 -> 0..127 */
            cc_value = state.axes[i] * 127 / 32767;
        }
        send_osc_cc(16, 100 + i, cc_value);
    }
}
