...
# trax.sh - Two-player tank game with animated treads
#
# =============================================================================
# TANK STATE MODEL
# =============================================================================
...
# Module paths
TRAX_SRC="${TRAX_SRC:-$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)}"
TDS_SRC="${TDS_SRC:-$TRAX_SRC/../tds}"
...
# Total display size (arena + borders + REPL) - fits 60x24 terminal
DISPLAY_WIDTH=$((ARENA_WIDTH + 2))    # 2 border chars
DISPLAY_HEIGHT=$((ARENA_HEIGHT + 8))  # 2 border + 2 score/status + 4 REPL lines
REPL_LINES=4
...
render_help() {
    ...
    printf '\n'
    printf '  TRAX - Two Player Tank Game\n'
    printf '  =============================\n'
    printf '\n'
...
process_input() {
    ...
    case "$key" in
        ...
        "\`"|"~")  # Easter egg: toggle debug HUD
            if [[ "${GAME_STATE[debug_hud]}" == "true" ]]; then
                GAME_STATE[debug_hud]="false"
            else
                GAME_STATE[debug_hud]="true"
            fi
            ;;
...
TRAX_FIFO="${TRAX_FIFO:-/tmp/trax_input}"
TRAX_USE_FIFO="${TRAX_USE_FIFO:-false}"
SAVED_STTY=""
...
setup_input() {
    ...
    if [[ "$TRAX_USE_FIFO" == "true" ]]; then
        [[ -p "$TRAX_FIFO" ]] || mkfifo "$TRAX_FIFO" 2>/dev/null
        exec 3<>"$TRAX_FIFO"
    fi
}
...
trax_repl() {
    ...
    while [[ "${GAME_STATE[running]}" == "true" ]]; do
        ...
        if [[ "${GAME_STATE[debug_hud]}" == "true" ]]; then
            ...
            [[ "${PLAYER_LEFT[velocity]}" == "0" ]] && p1_status="stopped" || p1_status="speed ${PLAYER_LEFT[velocity]}"
            [[ "${PLAYER_RIGHT[velocity]}" == "0" ]] && p2_status="stopped" || p2_status="speed ${PLAYER_RIGHT[velocity]}"
            repl_set 0 "P1[WASD]: ${p1_status}  P2[IJKL]: ${p2_status}"
            repl_set 1 ""
        fi
