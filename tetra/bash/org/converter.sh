...
tetra_convert_with_mapping() {
    ...
    # Parse mapping file
    local org_name
    org_name=$(jq -r '.org_name' "$mapping_file")

    echo "Converting infrastructure for organization: $org_name"
    echo "  Source: $json_file"
    echo "  Mapping: $mapping_file"
    echo "  Output: $output_file"
    echo ""

    # Extract base domain from JSON
    local base_domain
    base_domain=$(jq -r '.[] | select(.Domains) | .Domains[0].name' "$json_file" 2>/dev/null)

    # Start generating TOML
    cat > "$output_file" << EOF
# ${org_name} Organization Configuration
# Generated from DigitalOcean data on $(date -u '+%Y-%m-%dT%H:%M:%SZ')
# Tetra Endpoint Specification (TES) v2.0 compliant
...
# ═══════════════════════════════════════════════════════════
# TES SYMBOLS (Level 0: Semantic Labels)
# Maps deployment stages to infrastructure addresses
# ═══════════════════════════════════════════════════════════

[symbols]
EOF

    # Generate symbols from mapping
    echo "# Symbol resolution: Stage → Address" >> "$output_file"
    jq -r '.environments | to_entries[] |
        if .value.type == "localhost" then
            "\"@\(.key)\" = { address = \"\(.value.address)\", type = \"local\" }"
        else
            "\"@\(.key)\" = { address = \"\(.value.address)\", droplet = \"\(.value.droplet_name)\", type = \"remote\" }"
        end' "$mapping_file" >> "$output_file"

    # Generate connectors section (TES Level 3)
    cat >> "$output_file" << EOF

# ═══════════════════════════════════════════════════════════
# TES CONNECTORS (Level 3: Authenticated Channels)
# Dual-role authentication: auth_user:work_user@host
# ═══════════════════════════════════════════════════════════
...
# Physical resources from DigitalOcean
# ═══════════════════════════════════════════════════════════
...
EOF
    ...
    # Add infrastructure details from mapping
    for env in dev staging prod; do
        local env_data
        env_data=$(jq -r ".environments.$env // empty" "$mapping_file" 2>/dev/null)

        if [[ -n "$env_data" && "$env_data" != "null" ]]; then
            local droplet_name droplet_id address private_ip floating_ip domain
            droplet_name=$(echo "$env_data" | jq -r '.droplet_name')
            droplet_id=$(echo "$env_data" | jq -r '.droplet_id')
            address=$(echo "$env_data" | jq -r '.address')
            private_ip=$(echo "$env_data" | jq -r '.private_ip // ""')
            floating_ip=$(echo "$env_data" | jq -r '.floating_ip // ""')
            domain=$(echo "$env_data" | jq -r '.domain // ""')

            echo "" >> "$output_file"
            echo "# ${env^} Environment - $droplet_name" >> "$output_file"
            echo "${env}_droplet_id = $droplet_id" >> "$output_file"
            echo "${env}_droplet_name = \"$droplet_name\"" >> "$output_file"
            echo "${env}_address = \"$address\"" >> "$output_file"
            [[ -n "$private_ip" ]] && echo "${env}_private_ip = \"$private_ip\"" >> "$output_file"
            [[ -n "$floating_ip" && "$floating_ip" != "null" ]] && echo "${env}_floating_ip = \"$floating_ip\"" >> "$output_file"
            [[ -n "$domain" && "$domain" != "null" ]] && echo "${env}_domain = \"$domain\"" >> "$output_file"
        fi
    done
...
EOF
    ...
    for env in dev staging prod; do
        ...
        if [[ -n "$env_data" && "$env_data" != "null" ]]; then
            local address private_ip domain
            address=$(echo "$env_data" | jq -r '.address')
            private_ip=$(echo "$env_data" | jq -r '.private_ip // ""')
            domain=$(echo "$env_data" | jq -r '.domain // ""')

            # Infer domain if not in mapping
...
EOF
    ...
    for env in dev staging prod; do
        ...
        if [[ -n "$env_data" && "$env_data" != "null" ]]; then
            ...
            echo "$env = \"$domain\"" >> "$output_file"
        fi
    done

    echo "" >> "$output_file"
    echo "✅ Generated TES-compliant TOML: $output_file"
    return 0
}
